...
def get_iemocap_split(split_utterances):
    ...
    for vid in tqdm(train_vids, desc="train"):
        for split_i in range(len(video_text[vid]) - 1, split_utterances - 1, -1):
            train.append(
                cogmen.Sample(
                    vid,
                    video_speakers[vid][split_i - split_utterances + 1 : split_i + 1],
                    video_labels[vid][split_i - split_utterances + 1 : split_i + 1],
                    video_text[vid][split_i - split_utterances + 1 : split_i + 1],
                    video_audio[vid][split_i - split_utterances + 1 : split_i + 1],
                    video_visual[vid][split_i - split_utterances + 1 : split_i + 1],
                    video_sentence[vid][split_i - split_utterances + 1 : split_i + 1],
                )
            )
    for vid in tqdm(dev_vids, desc="dev"):
        for split_i in range(len(video_text[vid]) - 1, split_utterances - 1, -1):
